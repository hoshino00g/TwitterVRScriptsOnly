...
public class ProduceTweet : MonoBehaviour {
	...
	//string型のresponseにはTwitterRequestのrequest.downloadHandler.textが入る
	{
		if (success) {
			...
			for (int i = 0; i <= 190; i++) {
				...
				GameObject Tcolor = tweetprefab.transform.Find("Panel").gameObject;
				Tcolor.GetComponent<Image> ().color = new Color (Random.Range (0f, 1.0f), Random.Range (0f, 1.0f), Random.Range (0f, 1.0f), 0.6f);
				//アイコン画像の表示
				...
				GameObject j = tweetprefab.transform.Find ("ReTweet/ReTweetNum").gameObject;
				...
				GameObject k = tweetprefab.transform.Find("Favorite/FavoriteNum").gameObject;;
				Text favoriteT = k.GetComponent<Text> ();
				favoriteT.text = favorite;
				//ツイート時刻の表示
				string time = Response.items [i].created_at;
				GameObject t = tweetprefab.transform.Find ("Time").gameObject;
				Text timeT = t.GetComponent<Text> ();
				timeT.text = time;
				//以下はツイートテキストの表示
				//Debug.Log (Response.items [i].text);
				GameObject l = tweetprefab.transform.Find ("TweetText").gameObject;
				Text tweetLabel = l.GetComponent<Text> ();
				if (Response.items [i].text.Length > 32) {
					GameObject m = tweetprefab.transform.Find ("TweetText").gameObject;
					...
					if (Response.items [i].text.Length > 66) {
						GameObject n = tweetprefab.transform.Find ("TweetText").gameObject;
	...
	IEnumerator IconImageRepresent (string own,int i)
	{
		...
		WWW www = new WWW(own);
		yield return www;
		GameObject o = tweetprefab.transform.Find ("IconImage").gameObject;
