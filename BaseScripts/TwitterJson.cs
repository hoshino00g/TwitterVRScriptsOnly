...
namespace Twitter {
    ...
    //twitter内の機能を使う際のための変数を定義
    public class TwitterJson : MonoBehaviour
   {

        #region Basic Class
        //エディタ上で折りたたむ（一時的に非表示にすることができる。）,(vidual stadioのみ）、どのあたりにコードを描くかを決めることができる。
        //#endregionで末尾をくくる
        [Serializable]
        //クラスをシリアル化する・・・あるオブジェクトをファイルに格納するためバイトストリームに変換する過程のこと
        //スクリプト上でできたインスタンスをファイル等に格納したいときに用いられる。
        //この場合TweetObjectクラスは何らかの形で格納される
        ...
        {   
            ...
            //bool型の変数・・・この変数によって真偽を判定する場合に用いられる。if文でよく使われる
